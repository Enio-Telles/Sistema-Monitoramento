...
class AggregationService:
    ...
    def load_editable_table(self, cnpj_dir: Path, cnpj: str) -> Path:
        ...
        if not source.exists():
            raise FileNotFoundError("A tabela de origem para agregação não foi encontrada.")
    ...
    def build_aggregated_row(
        ...
        parsed_codes = self._parse_codigo_entries([row.get("lista_codigos") for row in rows])
        if not parsed_codes:
            raise ValueError("Não foi possível identificar códigos nas linhas selecionadas.")
        ...
        top_codes = [codigo for codigo, freq in freq_counter.items() if freq == max_freq]
        codigo_padrao = sorted(top_codes, key=natural_sort_key)[0]

        descricao = (descricao_resultante or self._pick_mode([row.get("descricao") for row in rows]) or rows[0].get("descricao") or "").strip()
        descricao_norm = (descricao_normalizada_resultante or normalize_text(descricao)).strip()
        ...
        descricao_padrao = rows[0].get("descricao_padrao") or rows[0].get("descrição_normalizada") or ""
        max_words = 0
        for row in rows:
        ...
        for row in rows:
            ...
            if not norms:
                norm = row.get("descrição_normalizada") or row.get("descricao_normalizada")
                if norm:
        ...
        merged_codigo_entries = [f"[{codigo}; {original_entries[codigo]}]" for codigo in sorted(original_entries, key=natural_sort_key)]
        aggregated = {
            ...
            "tipo_item_padrao": self._pick_mode([row.get("tipo_item_padrao") for row in rows]),
    ...
    def aggregate_rows(
        ...
        if "descricao_padrao" not in current.columns:
            ...
            norm_col = "descrição_normalizada" if "descrição_normalizada" in current.columns else "descricao_normalizada"
        ...
        return AggregationResult(target_path=target, aggregated_row=aggregated_row, removed_keys=removed_keys)

    def _append_log(self, cnpj: str, target: Path, rows: list[dict[str, Any]], aggregated_row: dict[str, Any]) -> None:
        payload = {
