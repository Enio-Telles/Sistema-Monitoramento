...
target_dir = Path("/tmp/cnpj_dir_3")
...
start = time.time()
# Better new approach using is_in
if not removed_keys:
    kept_df = current
else:
    # Convert tuples to string combinations for is_in
    # This avoids join overhead
    removed_keys_str = pl.Series([f"{k[0]}|{k[1]}" for k in removed_keys])

    # Create the same combination string in the dataframe
    mask = ~pl.concat_str([
        pl.col("descrição_normalizada").fill_null("").cast(pl.String),
        pl.lit("|"),
        pl.col("descricao").fill_null("").cast(pl.String)
    ]).is_in(removed_keys_str)

    kept_df = current.filter(mask)

kept_rows_new = kept_df.to_dicts()
end = time.time()
print(f"is_in Approach Time taken: {end - start:.4f} seconds")
