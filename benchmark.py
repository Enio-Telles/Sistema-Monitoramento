...
n_rows = 100000
df = pl.DataFrame({
    "descrição_normalizada": [f"norm_{i}" for i in range(n_rows)],
    "descricao": [f"desc_{i}" for i in range(n_rows)],
...
# Rows to remove
rows_to_remove = [
    {"descrição_normalizada": f"norm_{i}", "descricao": f"desc_{i}", "lista_codigos": [f"[{i}; 1]"]}
...
start = time.time()
service.aggregate_rows(target_dir, "123", rows_to_remove, "new desc", "new norm")
end = time.time()
